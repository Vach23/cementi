...
router.get('/', function (req, res) {
    ...
    <script>
    // Hero slideshow: only the first slide is in HTML. The rest are created on demand by JS
    // to avoid downloading 17 images on initial page load.
    (function() {
        ...
        // Preload the second slide immediately so the first transition is instant
        preloadNext(1);
    ...
    // Story-photos: click any thumb → open lightbox with the FULL pool (not just visible 4).
    // Slowly rotate the 4 visible thumbs through the pool so all photos eventually appear.
    // Wrapped in DOMContentLoaded because this script tag sits above the .story-photos element in markup.
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Click a thumb → open lightbox with the whole pool, starting at that photo's pool index.
        ...
        // Rotation: only useful when there are more pool photos than visible slots.
        if (pool.length <= slots.length) return;
        var nextPoolIdx = slots.length;     // first photo not yet shown
        var slotToReplace = 0;               // round-robin slot
        setInterval(function () {
